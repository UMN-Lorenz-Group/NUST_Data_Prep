...
Usage:
    ...
    python extract_nust_xlsx.py --dir ./NUST_Historical_Data_1941_1988/ --out_dir ./output/
...
ALL_TP_MARKERS = ("tp1", "tp2", "tp3a", "tp3b", "tp3c", "tp4", "tp5") + PER_LOC_MARKERS
...
_FUSED_MARKER_MAP: dict[str, str | None] = {
    ...
    "tp5a":          None,   # multi-year mean variant of tp5 — skip
    "tp5b":          None,   # multi-year mean variant of tp5 — skip
    "tp5c":          None,   # multi-year mean variant of tp5 — skip
    "tp5d":          None,   # multi-year mean variant of tp5 — skip
    "tp3":           "tp3a", # bare tp3 (no suffix) = descriptive/morphological block → treat as tp3a
    "tp2a":          "tp3a", # 1979 variant: combined descriptive/morphological section (Descriptive Code + Chlorosis + Shattering) → treat as tp3a
}
...
def call_claude(client: anthropic.Anthropic, cell_grid: str, group_label: str,
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            # 1. Try ```json ... ``` block (may have preamble text before the fence)
            fence_m = re.search(r"```(?:json)?\s*(\{.*\})\s*```", raw, re.DOTALL)
            if fence_m:
                raw = fence_m.group(1)
            else:
                # 2. Strip leading/trailing fences anchored at start/end
                raw = re.sub(r"^```(?:json)?\s*", "", raw)
                raw = re.sub(r"\s*```$", "", raw)
                # 3. If there's still preamble, find first { and trim
                brace = raw.find("{")
                if brace > 0:
                    raw = raw[brace:]
