# NUST Phenotype Processing (year-parameterized)
if (!exists("cfg")) {
  source("C:/Users/vramasub/Desktop/UMN_GIT/NUST_Data_Prep/data_prep/stage1_processing/nust_utils.R")
  source("C:/Users/vramasub/Desktop/UMN_GIT/NUST_Data_Prep/data_prep/stage1_processing/nust_config.R")
  cfg <- nust_config(
    year     = "2025",
    data_dir = "C:/Users/vramasub/Desktop/UMN_Projects/NUST_Projects/NUST_Data/2025/2025_NUST_Processing/"
  )
  setwd(cfg$data_dir)
}
...
# Read lookup table
Cal_Julian_Lookup <- read.csv(cfg$cal_file, header = TRUE)
...
Phenotypes_SdTr_DF_Filt[,"UniqueID1"] <- gsub(paste0(Year,"[-_]Seed Traits "),paste0(Year,"_"),Phenotypes_SdTr_DF_Filt[,"UniqueID1"])
...
Phenotypes_AgSd_Tr_Filt_DF <- as.data.frame(Phenotypes_AgSd_Tr_Filt_DF)
...
Data_Location_Names <- unique(Phenotypes_AgSd_Tr_Filt_DF[,"Location"])
...
# Location column dropped — City + State carry the location information
selCols1 <- c("Strain","Year","Test","City","State","OriginalStrain")
...
# Define year and test sets (re-using cfg values)
Year <- cfg$year
Tests1 <- cfg$Tests1
Tests2 <- cfg$Tests2
Tests3 <- cfg$Tests3
