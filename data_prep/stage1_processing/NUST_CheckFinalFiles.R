...
strainsTab1 <- strainsTab1[, !colnames(strainsTab1) %in% "OriginalStrain", drop=FALSE]
...
chksTable <- chksTable[, !colnames(chksTable) %in% c("OriginalStrain", "RM"), drop=FALSE]
...
## Guard D: copy metaTable1.csv to Files4Upload/ (written by NUST_HistProcessing.R)
meta_src <- "metaTable1.csv"
if (file.exists(meta_src))
  file.copy(meta_src, file.path(output_dir, "metaTable1.csv"), overwrite = TRUE)
