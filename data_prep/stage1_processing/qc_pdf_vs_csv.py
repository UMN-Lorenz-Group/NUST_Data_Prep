...
Usage:
    python qc_pdf_vs_csv.py --year 1980 --csv_dir ./output_files/output_1980/ --out_dir ./output_files/output_1980/qc/
    python qc_pdf_vs_csv.py --pdf "R:/.../1980_done.pdf" --csv_dir ./output_files/output_1980/ --out_dir ./output_files/output_1980/qc/
    python qc_pdf_vs_csv.py --year 1980 --csv_dir ./output_files/output_1980/ --flagged ./output_files/output_1980/validated/combined_1980_phenotypes_review_flagged.csv --out_dir ./output_files/output_1980/qc/ --mode flagged
    python qc_pdf_vs_csv.py --year 1980 --csv_dir ./output_files/output_1980/ --flagged ./output_files/output_1980/validated/combined_1980_phenotypes_review_flagged.csv --out_dir ./output_files/output_1980/qc/ --mode both
  ...
  values — Cell-level verification of every extracted trait value against the PDF.
    ...
    python qc_pdf_vs_csv.py --year 1980 --csv output_files/output_1980/validated/combined_1980_phenotypesTable_approved.csv --out_dir ./output_files/output_1980/qc/ --mode values
    python qc_pdf_vs_csv.py --year 1980 --csv output_files/output_1980/validated/combined_1980_phenotypesTable_approved.csv --out_dir ./output_files/output_1980/qc/ --mode values --test_filter UT-00
    python qc_pdf_vs_csv.py --year 1980 --csv output_files/output_1980/validated/combined_1980_phenotypesTable_approved.csv --out_dir ./output_files/output_1980/qc/ --mode values --resume
    python qc_pdf_vs_csv.py --year 1980 --csv output_files/output_1980/validated/combined_1980_phenotypesTable_approved.csv --out_dir ./output_files/output_1980/qc/ --mode values --dry_run
"""
...
Note on formatting: If a numeric value matches ignoring a trailing ".0" (e.g. "3" vs
"3.0") or a trailing "*" suffix (e.g. "9*" vs "9"), do NOT list it as a discrepancy.

Note on table selection: NUST annual reports contain multiple tables per test section.
Use ONLY the current-year single-year per-location tables. Do NOT read values from
multi-year mean tables (labeled "1980-1981, 2-year mean", "1979-1981, 3-year mean", etc.).
If a location appears as a column in a wide summary table, use the individual per-location
table instead (where each row is a strain and each column is a location).
...
def run_roster(client: anthropic.Anthropic, pdf_b64: str, year: str,
    ...
    # Load combined phenotypes CSV — prefer the TEST_MAP-applied table
    for _candidate in [
        csv_dir / f"combined_{year}_phenotypesTable.csv",
        csv_dir / f"validated" / f"combined_{year}_phenotypesTable_approved.csv",
        csv_dir / f"combined_{year}_phenotypes.csv",
        csv_dir / f"combined_{year}_phenotypes_raw.csv",
    ]:
        if _candidate.exists():
            pheno_path = _candidate
            break
    else:
...
def run_values(
    ...
    if city_filter:
        combos = combos[combos["City"] == city_filter].reset_index(drop=True)
        print(f"  Filtered to city: {city_filter} ({len(combos)} combos)", flush=True)
...
def main():
    ...
    parser.add_argument("--city_filter", type=str, default=None,
                        help="values mode: restrict to one city name, e.g. Rosemount")
