...
read_hist <- function(..., fallback_pattern) {
  candidates <- c(...)
  for (p in candidates) {
  ...
  if (!length(f)) stop(sprintf("[HistProcessing] No file for pattern '%s' in %s",
...
desc_raw    <- read_hist(sprintf("combined_%s_descriptive.csv",    YEAR),
                         fallback_pattern = "_descriptive\\.csv$")
...
pheno_vals <- unique(p$Phenotype)
if (any(pheno_vals %in% names(PHENO_MAP))) {
  # Extraction-style names → map through PHENO_MAP
  p <- p[p$Phenotype %in% names(PHENO_MAP), ]
  p$PhenoCol <- PHENO_MAP[p$Phenotype]
} else {
  # Already R column names (combined/validated pipeline output) — use directly
  p <- p[p$Phenotype %in% unname(PHENO_MAP), ]
  p$PhenoCol <- p$Phenotype
}
...
if (file.exists(combined_checks_path)) {
  ...
  # Normalize check strain names to match strainsTable/phenotypesTable conventions:
  # strip MG-qualifier parentheticals (0), (00), (I)-(IV) then strip all spaces
  normalize_check_strain <- function(x) {
    x <- gsub("\\s*\\((0{1,2}|I{0,3}V?|IV)\\)\\s*", "", x)
    x <- gsub(" ", "", x)
    x
  }
  checks_out$Strain         <- normalize_check_strain(checks_out$Strain)
  checks_out$OriginalStrain <- normalize_check_strain(checks_out$OriginalStrain)
